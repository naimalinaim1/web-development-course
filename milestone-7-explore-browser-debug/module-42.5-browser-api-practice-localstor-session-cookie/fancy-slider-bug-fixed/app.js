...
const showImages = (images) => {
  const noDataCon = document.getElementById("invalid-data");
  if (images.length == 0) {
    noDataCon.classList.remove("d-none");
    return;
  } else {
    noDataCon.classList.add("d-none");
  }
...
var timer;
const createSlider = (isInvalidDuration) => {
...
const sliderCall = () => {
  const duration = document.getElementById("duration").value * 1000 || 1000;
  const invalidTime = document.getElementById("invalid-time");
  if (duration < 0) {
    invalidTime.classList.remove("d-none");
  } else {
    invalidTime.classList.add("d-none");
    createSlider();
  }
};
